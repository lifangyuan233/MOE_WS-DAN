...
class CGCBlock(nn.Module):
    """层级化专家模块 (包含任务专属和共享专家)"""
    # 64/128   128  2  1  3
    def __init__(self, in_channels, expert_channels, num_task_experts=1, GateNum=2):
        ...
        self.gateA = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(128, num_task_experts),
            nn.Softmax(dim=1))
    ...
    def forward(self, x_A, x_B):
        ...
        # 组合专家输出
        combined_A = torch.stack(experts_A_out, dim=1)  # (bs, num_total, ec, h, w)
        Gate_A_out = (combined_A * gate_weights_A).sum(dim=1)

        
        combined_B = torch.stack(experts_B_out, dim=1)
...
class DeepExpertNetwork(nn.Module):
    """深度多专家网络"""
    def __init__(self, in_channels=3, base_channels=64, expert_channels=128, num_blocks=2, num_task_experts=2, mode='train'):
        ...
        for i in range(num_blocks):
            ...
            block = CGCBlock(in_channels=current_channels, expert_channels=expert_channels, num_task_experts=num_task_experts, GateNum=2) # 最后一层只用任务门控
